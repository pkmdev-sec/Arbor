...
// Module-scope memory peak tracking
const _memoryPeaks = new Map();
 ...
 * @param {number} stdoutBytes - Current stdout size in bytes
 * @param {number} stderrBytes - Current stderr size in bytes
 * @returns {object} Memory statistics { peakBufferSize, currentSize, exceeded }
 */
export function trackMemoryUsage(agentId, stdoutBytes = 0, stderrBytes = 0) {
  const currentSize = stdoutBytes + stderrBytes;

  const existing = _memoryPeaks.get(agentId) || 0;
  const peakBufferSize = Math.max(existing, currentSize);
  _memoryPeaks.set(agentId, peakBufferSize);
...
export function getMemoryStats(agentId) {
  if (!_memoryPeaks.has(agentId)) {
    return null;
  }

  const peakBufferSize = _memoryPeaks.get(agentId);
  return {
    peakBufferSize,
