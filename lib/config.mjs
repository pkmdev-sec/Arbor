...
// QW4: Role-specific thinking tokens (8K-32K range)
export const ROLE_THINKING_TOKENS = {
  worker: 8192,          // 8K — action-oriented, quick execution
  verifier: 32768,       // 32K — needs deep adversarial analysis
  decomposer: 16384,     // 16K — needs planning depth
  'sub-coordinator': 16384,  // 16K — coordination tasks
  governor: 16384,       // 16K — governance and oversight
  aggregator: 16384,     // 16K — result aggregation
};

export const DEFAULT_THINKING_TOKENS = 16384;  // 16K — safe default for unknown roles

// QW5: Role-specific output tokens (8K-64K range)
export const ROLE_OUTPUT_TOKENS = {
  worker: 65536,         // 64K — code generation needs space
  verifier: 16384,       // 16K — verdicts are concise
  decomposer: 8192,      // 8K — task lists are small
  'sub-coordinator': 16384,  // 16K — coordination messages
  governor: 16384,       // 16K — governance reports
  aggregator: 32768,     // 32K — aggregated results
};

export const DEFAULT_OUTPUT_TOKENS = 32768;  // 32K — safe default

// QW6: Role-specific bash output length (prevents truncated test/build output)
export const ROLE_BASH_OUTPUT_LENGTH = {
  worker: 200000,          // 200K — full test/build output
  verifier: 100000,        // 100K — moderate output
  decomposer: 50000,       // 50K — minimal bash usage
  'sub-coordinator': 100000,  // 100K — coordination tasks
  governor: 50000,         // 50K — governance tasks
  aggregator: 100000,      // 100K — result aggregation
};

export const DEFAULT_BASH_OUTPUT_LENGTH = 100000;  // 100K — safe default
...
export const SPECIALIST_SUPPLEMENTS = {
  frontend: 'SPECIALIST ROLE: Frontend. Focus on UI components, styling, accessibility, client-side state, browser APIs.',
  backend: 'SPECIALIST ROLE: Backend. Focus on APIs, databases, server logic, authentication, data models.',
  testing: 'SPECIALIST ROLE: Testing. Focus on test coverage, edge cases, mocking, regression prevention.',
  security: 'SPECIALIST ROLE: Security. Focus on input validation, auth flows, injection risks, data exposure.',
};
...
export function validateConfig(config, schema, filename) {
  ...
  function validateField(value, fieldSchema, path, seen = new WeakSet()) {
    // Type validation
    if (fieldSchema.type === "object") {
      ...
      if (seen.has(value)) {
        throw new Error(`[${filename}] Circular reference detected at ${path}`);
      }
      seen.add(value);
      ...
      if (fieldSchema.properties) {
        for (const [key, subSchema] of Object.entries(fieldSchema.properties)) {
          if (key in value) {
            validateField(value[key], subSchema, `${path}.${key}`, seen);
