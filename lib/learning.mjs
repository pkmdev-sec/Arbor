...
export default class LearningStore {
   ...
   * @param {string} [projectType=""] - Optional project type (e.g., "web_app", "cli_tool")
   */
  record(taskType, language, framework, pattern, projectType = "") {
    const key = this._makePatternKey(taskType, language, framework, projectType);
    const now = Date.now();
  ...
  /**
   * Query patterns with project type awareness and fallback.
   * First tries exact match with projectType, then falls back to base key without projectType.
   *
   * @param {string} taskType - Type of task
   * @param {string} language - Programming language
   * @param {string} framework - Framework/library
   * @param {string} projectType - Project type (e.g., "web_app", "cli_tool")
   * @returns {Array<object>} Relevant patterns, sorted by relevance
   */
  queryWithFallback(taskType, language, framework, projectType) {
    // Try exact match with projectType first
    const exactKey = this._makePatternKey(taskType, language, framework, projectType);
    const exactPattern = this.store.patterns[exactKey];

    if (exactPattern && exactPattern.status === "active") {
      const freshness = this._calculateFreshness(exactPattern.lastSeen);
      if (freshness >= MIN_FRESHNESS) {
        const relevance = exactPattern.count * freshness;
        return [{
          ...exactPattern,
          freshness,
          relevance,
          projects: Array.from(exactPattern.projects),
        }];
      }
    }

    // Fallback to base key without projectType
    const baseKey = this._makePatternKey(taskType, language, framework);
    const basePattern = this.store.patterns[baseKey];

    if (!basePattern || basePattern.status !== "active") {
      return [];
    }

    const freshness = this._calculateFreshness(basePattern.lastSeen);
    if (freshness < MIN_FRESHNESS) {
      return [];
    }

    const relevance = basePattern.count * freshness;

    return [{
      ...basePattern,
      freshness,
      relevance,
      projects: Array.from(basePattern.projects),
    }];
  }
  ...
  /**
   * Create pattern key from task type, language, framework, and optional project type.
   * @private
   */
  _makePatternKey(taskType, language, framework, projectType = "") {
    return projectType
      ? `${taskType}:${language}:${framework}:${projectType}`
      : `${taskType}:${language}:${framework}`;
  }
