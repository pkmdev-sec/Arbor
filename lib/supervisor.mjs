 ...
 * @param {string|null} config.scope       — file scope restriction (QW7)
...
export function buildClaudeArgs(config) {
  const {
    ...
    mcpConfigPath, persistContextDir, isRetry, previousResultFile, env, scope,
  } = config;
  ...
  // QW7: Session persistence — disable for scoped agents or when explicitly disabled
  // Scoped agents in worktrees should not create persistent session files
  if (!canResume || scope) {
    childArgs.push("--no-session-persistence");
  }

  // Session resume on retry (only if persistence is enabled)
  if (isRetry && canResume && !scope) {
    childArgs.push("--resume", sessionUUID, "--fork-session");
  } else if (canResume && !scope) {
