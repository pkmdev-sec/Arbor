...
import { createIpcStream } from './ipc-stream.mjs';
...
const MONITOR_TOPICS = [
  ...
  'telemetry', 'merge.started', 'merge.conflict', 'merge.completed',
];
...
function normalizeProtocolMessage(msg) {
  ...
  if (msg.payload?.type === 'monitor_message' && msg.payload?.originalMessage) {
    ...
    return normalizeProtocolMessage(original);
  }
