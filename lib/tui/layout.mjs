...
export function getLayoutDimensions(cols, rows) {
  ...
  switch (mode) {
    case 'full': {
      const sidebar = Math.max(30, Math.min(Math.floor(cols * 0.30), Math.floor(cols * 0.40)));
      return {
        mode,
        sidebarWidth: sidebar,
        mainWidth: cols - sidebar - 3, // -3 for borders
    ...
    case 'compact': {
      const sidebar = Math.max(30, Math.min(Math.floor(cols * 0.35), Math.floor(cols * 0.40)));
      return {
        mode,
        sidebarWidth: sidebar,
        mainWidth: cols - sidebar - 3,
