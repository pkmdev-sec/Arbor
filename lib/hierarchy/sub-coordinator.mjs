...
// ── Dynamic imports for optional integrations (graceful degradation) ────
let ContextBudget, CheckpointManager, AuditTrail, OutputValidator, WaveExecutor;
let routeModel, Premortem, DiscoveryChannel, getToolsForTask, filterContextForRole;
let generateCriteria, formatCriteriaForPrompt, evaluateCriteria;
...
try { ({ default: AuditTrail } = await import("../audit-trail.mjs")); } catch {}
try { ({ default: OutputValidator } = await import("../output-validator.mjs")); } catch {}
try { ({ default: WaveExecutor } = await import("../wave-executor.mjs")); } catch {}
try { ({ routeModel } = await import("../model-router.mjs")); } catch {}
try { ({ default: Premortem } = await import("../premortem.mjs")); } catch {}
try { ({ DiscoveryChannel } = await import("../discoveries.mjs")); } catch {}
try { ({ getToolsForTask } = await import("../tool-allowlists.mjs")); } catch {}
try { ({ filterContextForRole } = await import("../context-filter.mjs")); } catch {}
try { ({ generateCriteria, formatCriteriaForPrompt, evaluateCriteria } = await import("../task-criteria.mjs")); } catch {}
...
const DEFAULT_CONFIG = {
  ...
  /** Default agent budget for child spawns */
  defaultBudget: 5,
};
...
/**
 * Validate and normalize a parsed child result object (BUG #6).
 * Ensures required fields exist with safe defaults.
 *
 * @param {*} obj - Parsed JSON from child result file
 * @returns {Object} Normalized result object
 */
function validateChildResult(obj) {
  if (!obj || typeof obj !== "object") {
    return { status: "error", files: [], result: null, error: "Invalid result format" };
  }
...
export class SubCoordinator {
  ...
  constructor(config) {
    ...
    // ── Optional integration fields (all backward-compatible) ──

    /** @type {Object|null} Governor for budget enforcement */
    this._governor = config?.governor || null;

    /** @type {Object|null} Audit trail for invocation/result logging */
    this._auditTrail = config?.auditTrail || null;

    /** @type {Object|null} Discovery channel for cross-agent findings */
    this._discoveryChannel = config?.discoveryChannel || (DiscoveryChannel ? new DiscoveryChannel() : null);

    /** @type {Object|null} Checkpoint manager for crash recovery */
    this._checkpoint = (CheckpointManager && this.worktreeBase) ? new CheckpointManager(this.worktreeBase) : null;
  }
  ...
  async start() {
    try {
      ...
      // Integration 3: Audit trail — record invocation
      try {
        this._auditTrail?.recordInvocation(this.id, {
          task: this.task,
          level: this.level,
          scope: this.scope,
        });
      ...
      // Integration 2: Checkpoint — check for resume point
      try {
        const latest = this._checkpoint?.findLatest();
        if (latest) {
          this._log("info", "Found checkpoint for resume", { phase: latest.phase, phaseName: latest.phaseName });
        }
      } catch (_) { /* checkpoint is non-critical */ }
  ...
  async decompose() {
    try {
      ...
      // Integration 8: Premortem — filter wasteful subtasks
      let filteredSubtasks = subtasks;
      try {
        if (Premortem) {
          const pm = new Premortem();
          const { accepted, filtered } = pm.filter(subtasks);
          if (filtered.length > 0) {
            this._log("info", "Premortem filtered subtasks", { accepted: accepted.length, filtered: filtered.length });
          }
          filteredSubtasks = accepted;
        }
      } catch (_) { /* premortem is non-critical */ }

      // Integration 12: Task Criteria — generate criteria for each subtask
      try {
        if (generateCriteria && formatCriteriaForPrompt) {
          for (const st of filteredSubtasks) {
            const criteria = generateCriteria(st);
            st._criteria = criteria;
            st._criteriaPrompt = formatCriteriaForPrompt(criteria);
          }
        }
      } catch (_) { /* criteria is non-critical */ }

      // Allocate budget to subtasks
      const subtasksWithBudget = this._allocateBudget(filteredSubtasks);
      ...
      // Integration 2: Checkpoint — save after decomposition
      try {
        this._checkpoint?.save(1, "decomposition", {
          subtasks: subtasksWithBudget,
          strategy: "split",
        });
      } catch (_) { /* checkpoint is non-critical */ }
  ...
  async spawnChildren(subtasks) {
    ...
    try {
      ...
      // Integration 6: Wave Execution — spawn children in waves if available
      const pendingChildren = Array.from(this.children.values())
        .filter(child => child.status === ChildStatus.PENDING);

      if (WaveExecutor && pendingChildren.length > 1) {
        try {
          const waveExec = new WaveExecutor({ maxConcurrentPerWave: 3 });
          await waveExec.execute(pendingChildren, async (child) => {
            // Integration 9: Discoveries — inject discoveries for wave 2+ children
            try {
              const discoveryCtx = this._discoveryChannel?.getContextInjection();
              if (discoveryCtx && child.subtask) {
                child.subtask._discoveryContext = discoveryCtx;
              }
            } catch (_) { /* discoveries injection is non-critical */ }

            await this._spawnChild(child);
            return { task: child.subtask, filesChanged: [] };
          });
        } catch (waveErr) {
          this._log("warn", "Wave execution failed, falling back to parallel spawn", { error: waveErr.message });
          const spawnPromises = pendingChildren.map(child => this._spawnChild(child));
  ...
  async _aiDecompose() {
    try {
      ...
      // Integration 4: Prompt Defense — sanitize task description
      let safeTask = this.task;
      try {
        if (sanitizeForPrompt) {
          safeTask = sanitizeForPrompt(this.task);
        }
      } catch (_) { /* prompt defense is non-critical */ }

      // Integration 1: Context Budget — allocate budget for prompt components
      let taskDescription = safeTask;
      let codeContext = this.files.slice(0, 100).join("\n") +
        (this.files.length > 100 ? `\n... and ${this.files.length - 100} more` : "");
      try {
        if (ContextBudget) {
          const budget = new ContextBudget();
          taskDescription = budget.allocate("task", taskDescription, 0.15);
          codeContext = budget.allocate("code_context", codeContext, 0.50);
          this._log("info", "Context budget applied", { utilization: budget.utilizationPct().toFixed(1) + "%" });
        }
      } catch (_) { /* context budget is non-critical */ }
      ...
      let systemPrompt = ROLE_PROMPTS.hierarchical_decomposer || ROLE_PROMPTS.decomposer;

      // Integration 11: Context Filter — filter context for decomposer role
      try {
        if (filterContextForRole) {
          const filteredCtx = filterContextForRole({ task: { description: this.task, scope: this.scope } }, "decomposer");
          if (filteredCtx) {
            this._log("info", "Context filtered for decomposer role");
          }
        }
      } catch (_) { /* context filter is non-critical */ }

      // Integration 7: Model Routing — select model for decomposition
      let decomposeModel = "claude-sonnet-4-6";
      try {
        if (routeModel) {
          const routing = routeModel({ type: "decomposition", description: this.task, targetFiles: this.files });
          decomposeModel = routing.model || decomposeModel;
          this._log("info", "Model routed for decomposition", { model: decomposeModel, reason: routing.reason });
        }
      } catch (_) { /* model routing is non-critical */ }
  ...
  async _spawnChild(child) {
    try {
      // BUG #2: Governor budget check before every child spawn
      if (this._governor) {
        try {
          this._governor.requestBudget(child.id, 1);
        } catch (govErr) {
          if (govErr.name === "GovernorLimitError") {
            this._log("warn", "Governor denied child spawn", { childId: child.id, error: govErr.message });
            child.status = ChildStatus.FAILED;
            child.error = `Governor limit: ${govErr.message}`;
            return;
          }
          throw govErr;
        }
      ...
      // Integration 3: Audit trail — record child spawn
      try {
        this._auditTrail?.recordInvocation(child.id, {
  ...
  async _spawnSubCoordinator(child) {
    try {
      ...
      // Integration 7: Model Routing — select model for complex sub-coordinator tasks
      let childModel = child.subtask.model || "sonnet";
      try {
        if (routeModel) {
          const routing = routeModel({ type: "implementation", description: child.subtask.task, targetFiles: child.subtask.scope || [] });
          childModel = routing.model || childModel;
        }
      } catch (_) { /* model routing is non-critical */ }

      // Integration 10: Tool Allowlists — get tools for coordinator role
      let tools;
      try {
        tools = getToolsForTask?.({ type: "coordinator" });
      } catch (_) { /* tool allowlists is non-critical */ }
      ...
      const subConfig = {
        ...
        // BUG #2: Pass governor to recursive sub-coordinator
        governor: this._governor,
        // Integration 9: Pass discovery channel to sub-coordinators
        discoveryChannel: this._discoveryChannel,
        // Integration 3: Pass audit trail
      ...
      const result = await spawnAgent({
        ...
        // BUG #3: Use computed budget instead of hardcoded 5
        budget: child.subtask?.budget || child.allocatedBudget || this.config.defaultBudget || 5,
        timeout: this.config.workerTimeout,
        resultFile: child.resultFile,
        agentId: child.id,
        cwd: child.worktreePath || this.mainCwd,
        ipcSocket: this.busAddress,
        tools,
      });
