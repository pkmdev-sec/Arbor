...
export async function aggregateProgress(scopedBuses) {
  ...
  for (const bus of scopedBuses) {
    ...
    try {
      // Request progress from each bus
      // This assumes agents implement a progress query endpoint
      const progress = await bus.request(bus.agentId, { type: "get_progress" }, 2000).catch(() => null);
      ...
      if (progress) {
        ...
        // If progress query failed, assume agent is active at 0%
        agentProgress.push({
          agentId: bus.agentId,
          scope: bus.scope,
          level: bus.level,
