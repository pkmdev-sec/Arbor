...
function buildSemanticMergePrompt(filePath, agents, baseContent, level) {
  ...
  let prompt = `# Semantic Merge Request\n\n`;
  prompt += `**File:** \`${filePath}\`\n`;
  prompt += `**Context:** ${levelContext}\n`;
  prompt += `**Agents involved:** ${agents.length}\n\n`;

  prompt += `## Base Version\n\n`;
  if (baseContent) {
    prompt += "```\n" + baseContent + "\n```\n\n";
  } else {
    prompt += "*No base version (new file)*\n\n";
  }
  ...
  for (let i = 0; i < agents.length; i++) {
    ...
    const content = agent.files.get(filePath);

    prompt += `### Agent ${i + 1} (${agent.agentId})\n\n`;
    prompt += `**Scope:** ${agent.scope.join(", ")}\n\n`;
    prompt += "```\n" + content + "\n```\n\n";
  }

  prompt += `## Task\n\n`;
  prompt += `Analyze the changes from all agents and produce a single merged version that:\n`;
  prompt += `1. Preserves all functional changes from each agent\n`;
  prompt += `2. Resolves any conflicts semantically (not just textually)\n`;
  prompt += `3. Maintains code consistency and style\n`;
  prompt += `4. Avoids duplicate or redundant code\n\n`;

  prompt += `Output the complete merged file content within a code fence:\n\n`;
  prompt += "```merged\n";
  prompt += "// Your merged content here\n";
  prompt += "```\n";
...
function fallbackToGitMerge(filePath, agents, mainCwd) {
  ...
  // Use first agent's version as "ours", last as "theirs"
  // (This is a simplification; proper implementation would merge pairwise)
  const ourContent = agents[0].files.get(filePath);
  const theirContent = agents[agents.length - 1].files.get(filePath);

  const result = performGitMerge(filePath, baseContent, ourContent, theirContent);

  return {
    content: result.content,
    confidence: result.hasConflicts ? 0.3 : 0.9,
    strategy: result.hasConflicts ? "git_merge_with_conflicts" : "git_merge_clean",
  };
}
  ...
  try {
    ...
    if (response.parsed && Array.isArray(response.parsed)) {
      for (const resolution of response.parsed) {
        resolutions.push({
          filePath: resolution.file || "unknown",
          strategy: resolution.strategy || "manual_review",
          confidence: resolution.confidence || 0.5,
          reasoning: resolution.reasoning || "LLM analysis",
        });
      }
    }
  } catch (error) {
