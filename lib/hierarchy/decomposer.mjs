  ...
  try {
    ...
    if (isAiClientAvailable() && task.length > 20) {
      try {
        ...
        if (aiResult.parsed) {
          if (aiResult.parsed.adjustedComplexity) {
            estimatedComplexity = aiResult.parsed.adjustedComplexity;
          }
          if (aiResult.parsed.adjustedDepth) {
            recommendedDepth = Math.max(1, Math.min(3, aiResult.parsed.adjustedDepth));
          }
          if (aiResult.parsed.adjustedFanOut) {
            recommendedFanOut = Math.max(2, Math.min(5, aiResult.parsed.adjustedFanOut));
          }
          if (aiResult.parsed.isGreenfield === true) {
            isGreenfield = true;
          }
          if (aiResult.parsed.targetPath && typeof aiResult.parsed.targetPath === "string") {
            targetPath = aiResult.parsed.targetPath;
          }

          logStructured("decomposer", "ai_adjustment", {
            reasoning: aiResult.parsed.reasoning || "no reasoning provided",
            isGreenfield,
            targetPath
          });
        }
...
async function decomposeGreenfieldTask(task, targetPath, config) {
  ...
  if (isAiClientAvailable()) {
    try {
      ...
      if (aiResult.parsed && Array.isArray(aiResult.parsed.concerns) && aiResult.parsed.concerns.length > 0) {
        concerns = aiResult.parsed.concerns.slice(0, config.maxFanOut || 5);
        logStructured("decomposer", "greenfield_ai_decompose", {
          concerns: concerns.map(c => c.name),
          reasoning: aiResult.parsed.reasoning || "no reasoning"
        });
      }
    } catch (err) {
...
export async function decomposeHierarchically(task, workDir, config = {}) {
  ...
  const cfg = {
    ...
    minFilesForSplit: config.minFilesForSplit || 4,
  ...
  try {
    ...
    if (leafCount === boundaries.length || couplingDensity > 2.5) {
      ...
      const children = boundaries.slice(0, Math.min(cfg.maxFanOut, cfg.maxTotalAgents - 1)).map((b, i) => ({
        id: `worker-${i + 1}`,
        type: "worker",
        level: 1,
...
function buildTreeRecursive(task, boundaries, level, config, remainingBudget) {
  ...
  if (splittable.length === 0) {
    const children = boundaries.slice(0, maxChildren).map((b, i) => ({
...
export function estimateAgentBudget(tree) {
  try {
    ...
    // Rough cost estimate: $0.015 per agent (avg sonnet call)
    const estimatedCost = totalAgents * 0.015;
