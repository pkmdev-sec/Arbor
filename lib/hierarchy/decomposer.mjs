...
import { aiJsonDecision, aiSchemaDecision, isAiClientAvailable } from "../ai-client.mjs";
...
// ── JSON Schemas for decomposer output ───────────────────────────────

// Schema for scope analysis (analyzeTaskScope)
const SCOPE_ANALYSIS_SCHEMA = {
  type: 'object',
  properties: {
    adjustedComplexity: { type: 'string', enum: ['small', 'medium', 'large'] },
    adjustedDepth: { type: 'number', minimum: 1, maximum: 3 },
    adjustedFanOut: { type: 'number', minimum: 2, maximum: 5 },
    isGreenfield: { type: 'boolean' },
...
// Schema for greenfield decomposition (decomposeGreenfieldTask)
  ...
  try {
    ...
    if (isAiClientAvailable() && task.length > 20) {
      try {
        ...
        // Check if schema enforcement is enabled (default: enabled, set ARBOR_SCHEMA_ENFORCE=0 to disable)
        const useSchemaEnforcement = process.env.ARBOR_SCHEMA_ENFORCE !== '0';
        const aiCallOpts = {
          model: selectedModel,
          system: [
        ...
        const aiResult = useSchemaEnforcement
          ? await aiSchemaDecision({ ...aiCallOpts, schema: SCOPE_ANALYSIS_SCHEMA })
          : await aiJsonDecision(aiCallOpts);
...
async function decomposeGreenfieldTask(task, targetPath, config) {
  ...
  if (isAiClientAvailable()) {
    try {
      ...
      const aiResult = useSchemaEnforcement
        ? await aiSchemaDecision({ ...aiCallOpts, schema: GREENFIELD_SCHEMA })
        : await aiJsonDecision(aiCallOpts);
