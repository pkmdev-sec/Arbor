...
function saveState() {
  ...
  try {
    ...
    writeFileSync(STATE_FILE, JSON.stringify(state, null, 2)); // Atomic on most systems
  } catch (err) {
...
function logEvent(event) {
  try {
    appendFileSync(IPC_LOG, JSON.stringify({ ...event, ts: Date.now(), agentId: AGENT_ID }) + '\n');
  } catch (err) {
...
function handleReportProgress(params) {
  ...
  saveState();
  logEvent({ type: 'progress', percent, step, files_touched });
...
function handleReportResult(params) {
  ...
  saveState();
  logEvent({ type: 'result', status, summary, files_modified, issues });
...
function handleLog(params) {
  ...
  saveState();
  logEvent({ type: 'log', level, message });
...
function main() {
  ...
  saveState();
  logEvent({ type: 'start', agentId: AGENT_ID, task: TASK, scope: SCOPE });
  ...
  const cleanup = () => {
    logEvent({ type: 'shutdown', agentId: AGENT_ID });
    process.exit(0);
  };
