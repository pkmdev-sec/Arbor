...
import { DEPTH, ROLE_PROMPTS, SPECIALIST_SUPPLEMENTS } from "./config.mjs";
...
import WaveExecutor from "./wave-executor.mjs";
...
function inferSpecialist(desc) {
  if (!desc) return null;
  const d = desc.toLowerCase();
  if (/\b(test|spec|assert|mock|coverage)\b/.test(d)) return 'testing';
  if (/\b(ui|css|component|frontend|style|layout|render)\b/.test(d)) return 'frontend';
  if (/\b(api|database|db|server|backend|auth|endpoint|migration)\b/.test(d)) return 'backend';
  if (/\b(security|vuln|inject|xss|csrf|sanitiz)\b/.test(d)) return 'security';
  return null;
}

// ── Phase: Decompose task into subtasks ───────────────────────────
export async function decompose(task, maxAgents, depth, contextFile, workDir, projectTree = "", options = {}) {
  const { busSocketPath = null } = options;
...
export async function executeParallel(subtasks, depth, contextFile, workDir, scoutSummary, busSocketPath = null, opts = {}) {
  ...
  // BUG FIX 5: Race condition protection - worktrees MUST be created sequentially
  // Git worktree operations have a filesystem lock, so parallel creation would fail.
  // This sequential loop prevents race conditions in:
  // - Worktree path allocation (each agent gets a unique path)
  // - Git worktree registry updates (git maintains internal state)
  // - Result file path generation (agent IDs are unique and sequential)
  // R2 Phase A: Create worktrees SEQUENTIALLY (git worktree lock)
  const agentSetups = [];
  const usedPaths = new Set();  // BUG FIX 5: Track allocated paths to detect collisions
  ...
  for (let i = 0; i < subtasks.length; i++) {
    ...
    try {
      ...
      // BUG FIX 5: Detect worktree path collisions (should never happen, but defensive)
      if (gitResult.wtPath && usedPaths.has(gitResult.wtPath)) {
        throw new Error(`Worktree path collision detected: ${gitResult.wtPath}`);
      }
      if (gitResult.wtPath) {
        usedPaths.add(gitResult.wtPath);
      }
  ...
  const spawnAgentByIndex = (i, waveKnowledge = []) => {
    ...
    // F11: Filter combined context for worker role
    let combinedPrompt = knowledgePrompts.length > 0 ? knowledgePrompts.join("\n\n") : null;
    if (combinedPrompt) {
      combinedPrompt = filterSystemPromptForRole(combinedPrompt, "worker");
    }

    // REQ-015: Inject learned prompt hints
    const _ls = new LearningStore();
    const _hints = _ls.getPromptHints('worker');
    if (_hints.length > 0) {
      combinedPrompt = (combinedPrompt || '') + '\n\nLEARNED FROM PRIOR RUNS:\n' + _hints.slice(0, 5).map(h => '- ' + h.hint).join('\n');
    }

    // REQ-009: Inject specialist supplement
    const _specialist = inferSpecialist(st.description);
    if (_specialist && SPECIALIST_SUPPLEMENTS[_specialist]) {
      combinedPrompt = (combinedPrompt || '') + '\n\n' + SPECIALIST_SUPPLEMENTS[_specialist];
    }

    // REQ-036: Inject domain supplement
    if (opts.domainSupplement) {
      combinedPrompt = (combinedPrompt || '') + '\n\nPROJECT CONTEXT:\n' + opts.domainSupplement;
    }
  ...
  // REQ-027: Continuous execution mode
  if (opts.continuous) {
    const we = new WaveExecutor({ maxConcurrentPerWave: Math.min(subtasks.length, 5) });
    const contResults = await we.executeContinuous(
      subtasks.map((s, i) => ({ ...s, id: 'agent-' + i })),
      async (task) => spawnAgentByIndex(subtasks.indexOf(task) >= 0 ? subtasks.indexOf(task) : 0, []),
      {
        generateFollowUps: (result) => {
          const lines = (result.output || '').split('\n');
          return lines.filter(l => l.startsWith('FOLLOW_UP:')).map(l => ({ description: l.slice(10).trim() }));
        },
        maxFollowUps: 3,
      }
    );
  ...
  // BUG FIX 7: Add timeout protection for entire swarm execution
  // Default: 2 hours for swarm (reasonable max for even large multi-agent tasks)
  // Individual agents have their own timeouts; this is a safety net for hung coordination
  const SWARM_TIMEOUT_MS = parseInt(process.env.ARBOR_SWARM_TIMEOUT_MS) || (2 * 60 * 60 * 1000);

  // Helper to create a timeout promise that rejects
  const createSwarmTimeout = (ms) => {
    return new Promise((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Swarm execution timed out after ${(ms / 1000 / 60).toFixed(1)} minutes`));
      }, ms);
    });
  };

  // Wrap the wave execution loop in a timeout race
  let swarmTimedOut = false;
  let swarmTimeoutError = null;
    ...
    await Promise.race([
      ...
      (async () => {
        for (let waveIdx = 0; waveIdx < waves.length; waveIdx++) {
          ...
          const settledResults = await Promise.allSettled(wavePromises);
          ...
          settledResults.forEach((result, idx) => {
            if (result.status === 'fulfilled') {
              fulfilled.push({ index: wave[idx], result: result.value });
            } else {
              rejected.push({ index: wave[idx], reason: result.reason });
              log(`${colors.red}✗ Agent ${agentSetups[wave[idx]].id} failed: ${result.reason?.message || result.reason}${colors.reset}`);
            }
          });

          // Store successful results in original order
          fulfilled.forEach(({ index, result }) => {
            results[index] = result;
          });

          // Prepare waveResults array for knowledge extraction (only successful ones)
          const waveResults = fulfilled.map(({ result }) => result);
      ...
      // Timeout
      createSwarmTimeout(SWARM_TIMEOUT_MS),
    ]);
  } catch (err) {
    if (err.message && err.message.includes("timed out")) {
      swarmTimedOut = true;
      swarmTimeoutError = err;
      log(`${colors.red}✗ Swarm execution timed out after ${(SWARM_TIMEOUT_MS / 1000 / 60).toFixed(1)} minutes${colors.reset}`);
      logIpc('orchestrator', 'all', 'error', `Swarm timeout: ${err.message}`, {});

      // Mark incomplete agents as timeout
      for (let i = 0; i < subtasks.length; i++) {
        if (!results[i]) {
          const setup = agentSetups[i];
          results[i] = {
            id: setup.id,
            subtask: setup.st.title || setup.st.task?.slice(0, 80),
            scope: setup.st.scope || [],
            model: setup.st.model || "sonnet",
            exitCode: -1,
            status: "timeout",
            output: `Agent did not complete before swarm timeout (${(SWARM_TIMEOUT_MS / 1000 / 60).toFixed(1)}min)`,
            durationMs: 0,
            resultFile: join(workDir, `${setup.id}-result.json`),
            isolation: { success: false },
          };
        }
      }
    } else {
  ...
  // BUG FIX 7: Continue with partial results if swarm timed out
  // (cleanup and validation still runs on completed agents)
  ...
  for (const r of results) {
    ...
    const apply = await validateAndApply(r.isolation.worktreePath, mainCwd, r.isolation.snapshot, r.isolation.backupDir, r.isolation.copiedUntracked, knownApplied, r.scope || [], mergedFiles, unresolvedConflicts, r.isolation.baseCommit);
    if (apply.valid) {
...
export async function executePipeline(task, depth, contextFile, workDir, busSocketPath = null, opts = {}) {
  const preset = DEPTH[depth];
  const mainCwd = process.cwd();
  ...
  for (const [i, stage] of stages.entries()) {
    // BUG FIX 4: Check if previous stage failed and abort pipeline if so
    // Pipeline stages should not run with corrupted input from failed predecessors
    if (i > 0 && results[i - 1]?.exitCode !== 0) {
      pipelineAborted = true;
      abortReason = `Stage ${i} (${results[i - 1].name}) failed with exit code ${results[i - 1].exitCode}`;
      log(`${colors.red}✗ Pipeline aborted: ${abortReason}${colors.reset}`);
      logIpc('orchestrator', 'all', 'error', `Pipeline aborted: ${abortReason}`, { failedStage: results[i - 1].name });

      // Mark remaining stages as skipped
      for (let j = i; j < stages.entries().length; j++) {
        const skippedStage = stages[j];
        results.push({
          id: `stage-${j + 1}-${skippedStage.name.toLowerCase()}`,
          name: skippedStage.name,
          subtask: skippedStage.task.slice(0, 80),
          model: skippedStage.model,
          exitCode: -1,
          status: "skipped",
          output: `Skipped due to pipeline abort: ${abortReason}`,
          durationMs: 0,
          resultFile: null,
          worktreePath: null,
        });
    ...
    // REQ-015 + REQ-036: Inject hints and domain supplement for pipeline
    let pipelineSystemPrompt = null;
    const _pls = new LearningStore();
    const _pHints = _pls.getPromptHints('worker');
    if (_pHints.length > 0) {
      pipelineSystemPrompt = 'LEARNED FROM PRIOR RUNS:\n' + _pHints.slice(0, 5).map(h => '- ' + h.hint).join('\n');
    }
    if (opts.domainSupplement) {
      pipelineSystemPrompt = (pipelineSystemPrompt || '') + '\n\nPROJECT CONTEXT:\n' + opts.domainSupplement;
    }
    ...
    // BUG FIX 4: Track validation failures as they also indicate stage failure
    ...
    // BUG FIX 4: Mark stage as failed if validation failed, even if exit code was 0
    const effectiveExitCode = validationFailed ? 1 : result.exitCode;

    results.push({
      id,
      name: stage.name,
      subtask: stage.task.slice(0, 80),
      model: stage.model,
      ...result,
      exitCode: effectiveExitCode,  // Override with validation failure
      resultFile: rf,
      worktreePath: isolation.worktreePath,
      validationFailed,  // Track validation failure for contract
    });
  }
...
export async function verify(task, workerResults, depth, workDir, busSocketPath = null) {
...
export function buildContract(task, mode, workerResults, verifyResult, totalMs, workDir, conflictReport = null, depth = "normal") {
  ...
  const agents = workerResults.map((r) => {
    ...
    // BUG FIX 6: Determine status more accurately to account for all failure modes
    let status = "completed";
    if (r.status && r.status !== "completed") {
      // Explicitly set status (e.g., "skipped", "killed", "timeout")
      status = r.status;
    } else if (r.exitCode === undefined || r.exitCode === null) {
      // Agent never completed (killed during shutdown, crashed)
      status = "killed";
    } else if (r.exitCode !== 0) {
      // Non-zero exit code
      status = "failed";
    } else if (r.validationFailed) {
      // Exit code was 0 but validation failed (from pipeline)
      status = "failed";
    }
    ...
    return {
      ...
      status,  // BUG FIX 6: More accurate status tracking
  ...
  // BUG FIX 6: Count all status types accurately
  const statusCounts = {
  ...
  const contract = {
    ...
    summary: {
      ...
      skipped: statusCounts.skipped,  // BUG FIX 6: Track skipped agents
      killed: statusCounts.killed,    // BUG FIX 6: Track killed agents
      timeout: statusCounts.timeout,  // BUG FIX 6: Track timed-out agents
...
// ── Auto-detect mode ──────────────────────────────────────────────
const VALID_MODES = new Set(["single", "parallel", "pipeline", "swarm", "hierarchical", "review", "fork-merge"]);
