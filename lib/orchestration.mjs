...
export async function executeParallel(subtasks, depth, contextFile, workDir, scoutSummary) {
  ...
  for (const r of results) {
    ...
    const apply = await validateAndApply(r.isolation.worktreePath, mainCwd, r.isolation.snapshot, r.isolation.backupDir, r.isolation.copiedUntracked, knownApplied, r.scope || []);
    if (apply.valid) {
