...
class BusClient:
    ...
    def __init__(
        ...
        self.socket_path = socket_path or os.environ.get(
            "CLAUDE_IPC_SOCKET", "/tmp/claude-ipc-bus.sock"
        )
    ...
    def request(
        ...
        request_id = f"{self.agent_id}-{self.next_request_id}"
        self.next_request_id += 1
...
def cli_main():
    ...
    if len(sys.argv) < 3:
        print(f"""Usage:
  {sys.argv[0]} send <target> '<json>'
  {sys.argv[0]} publish <topic> '<json>'
  {sys.argv[0]} request <target> '<json>' [timeout]
  {sys.argv[0]} subscribe <topic>""", file=sys.stderr)
        sys.exit(1)

    command = sys.argv[1]
    target_or_topic = sys.argv[2]
