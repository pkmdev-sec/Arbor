...
export class MessageBus {
  ...
  constructor(options = {}) {
    ...
    /** @type {Map<string, {timestamps: number[], windowStart: number}>} Agent ID → rate limit tracking (O(1) sliding window) */
    this.rateLimitWindows = new Map();

    /** @type {Map<string, RegExp>} Pattern → compiled regex cache */
  ...
  /**
   * Check if agent exceeds rate limit (O(1) sliding window with counter)
   * @private
  ...
  _checkRateLimit(agentId) {
    ...
    // Get or initialize rate limit tracking for this agent
    if (!this.rateLimitWindows.has(agentId)) {
      this.rateLimitWindows.set(agentId, { timestamps: [], windowStart: 0 });
    }
    const tracking = this.rateLimitWindows.get(agentId);

    // Check if we need to start a new window
    const cutoff = now - windowMs;
    if (tracking.timestamps.length === 0 || tracking.timestamps[tracking.windowStart] < cutoff) {
      // Find new window start using O(1) counter approach
      while (tracking.windowStart < tracking.timestamps.length &&
             tracking.timestamps[tracking.windowStart] < cutoff) {
        tracking.windowStart++;
      }

      // Cleanup old timestamps periodically to prevent unbounded growth
      if (tracking.windowStart > 100) {
        tracking.timestamps = tracking.timestamps.slice(tracking.windowStart);
        tracking.windowStart = 0;
      }
    }

    // Count messages in current window (O(1) with windowStart index)
    const messagesInWindow = tracking.timestamps.length - tracking.windowStart;
