  ...
  feed(chunk) {
    ...
    while (true) {
      ...
      if (this.expectedLength === null) {
        if (this._totalLength < 4) {
          break;
        }

        // Concat only when a complete message frame is ready (length prefix + payload)
        ...
        this._buffers = remaining.length > 0 ? [remaining] : [];
        this._totalLength = remaining.length;
      }

      // State 2: Waiting for payload
      if (this._totalLength < this.expectedLength) {
        break;
      }

      // Only concat when we have a complete message frame
