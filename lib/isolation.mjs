...
export async function cacheSnapshot(dir, excludePatterns = DEFAULT_EXCLUDES) {
  const snapshot = await snapshotFiles(dir, excludePatterns);
 ...
 */
export async function snapshotFiles(dir, excludePatterns = DEFAULT_EXCLUDES) {
  const filePaths = [];
  ...
  function walk(currentDir, relativeTo) {
    try {
      ...
      if (err.code !== 'ENOENT' && err.code !== 'EACCES') {
        process.stderr.write(`[isolation] unexpected error: ${err.message}\n`);
      }
    }
  }
  ...
  if (existsSync(nmSrc) && !existsSync(nmDst)) {
    ...
    if (!cowSuccess) {
      try {
        ...
        log(`${colors.dim}isolation: node_modules symlinked (CoW unavailable)${colors.reset}`);
      } catch (err) {
        if (err.code !== 'ENOENT' && err.code !== 'EACCES') {
          process.stderr.write(`[isolation] unexpected error: ${err.message}\n`);
        }
 ...
 * @param {string} options.worktreePath - Path to the agent's worktree
 * @param {string} options.mainCwd - Original working directory
 * @param {object} options.preSnapshot - Snapshot taken before agent ran
 * @param {string} options.backupDir - Directory with file backups for rollback
 * @param {string[]} options.copiedUntracked - Untracked files copied into worktree
 * @param {Set<string>} options.knownApplied - Files applied by prior agents (skip in escape detection)
 * @param {string[]} options.agentScope - Files in agent's claimed scope (always re-hash)
 * @param {Map<string, string>} options.mergedFiles - Merged file contents for conflict resolution
 * @param {Set<string>} options.unresolvedConflicts - Files with unresolved merge conflicts
...
export async function validateAndApply({ worktreePath, mainCwd, preSnapshot, backupDir, copiedUntracked = [], knownApplied = new Set(), agentScope = [], mergedFiles = new Map(), unresolvedConflicts = new Set() }) {
  ...
  try {
    ...
    // Warn if agent modified package manifest — worktree node_modules may be stale
