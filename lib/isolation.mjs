...
import { readFileSync, statSync, existsSync, readdirSync, mkdirSync, copyFileSync, unlinkSync, rmSync, symlinkSync } from "node:fs";
 ...
 */
export async function validateAndApply(worktreePath, mainCwd, preSnapshot, backupDir, copiedUntracked = [], knownApplied = new Set(), agentScope = []) {
  const applied = [];
  const escaped = [];
  const rolled_back = [];
  const errors = [];
