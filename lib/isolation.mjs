...
export function prepareWorktreeGit(workDir, agentId, mainCwd) {
  ...
  // Capture base commit SHA before creating worktree
  ...
  return { wtPath, copiedUntracked, backupDir, baseCommit };
}
 ...
 * @param {string} baseCommit - Base commit SHA to diff against (null = HEAD)
 * @returns {{ valid, applied, escaped, rolled_back, errors }}
 */
export async function validateAndApply(worktreePath, mainCwd, preSnapshot, backupDir, copiedUntracked = [], knownApplied = new Set(), agentScope = [], mergedFiles = new Map(), unresolvedConflicts = new Set(), baseCommit = null) {
  const applied = [];
  const escaped = [];
  const rolled_back = [];
  const errors = [];
  ...
  try {
    ...
    // Use baseCommit as the diff reference to detect staged/committed changes
    const baseRef = baseCommit || "HEAD";
    const [diffResult, delResult, addResult, untrackedResult] = await Promise.all([
      execFileAsync("git", ["diff", baseRef, "--name-only"], { cwd: worktreePath, encoding: "utf-8", timeout: 10000 }).catch(() => ({ stdout: "" })),
      execFileAsync("git", ["diff", baseRef, "--name-only", "--diff-filter=D"], { cwd: worktreePath, encoding: "utf-8", timeout: 10000 }).catch(() => ({ stdout: "" })),
      execFileAsync("git", ["diff", baseRef, "--name-only", "--diff-filter=A"], { cwd: worktreePath, encoding: "utf-8", timeout: 10000 }).catch(() => ({ stdout: "" })),
    ...
    const validationPromises = allChangedFiles.map(({ file, cwd }) => {
      if (/\.(mjs|js|cjs)$/.test(file)) {
        ...
        return execFileAsync("python3", ["-c", "import sys, py_compile; py_compile.compile(sys.argv[1], doraise=True)", fullPath], { timeout: 10000 })
          .then(() => null)
          .catch(err => `${file}: ${(err.message || "validation failed").split("\n")[0]}`);
      }
