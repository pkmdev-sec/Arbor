...
// IPC channel for lifecycle events (optional, non-blocking)
let ipcChannel = null;

/**
 * Initialize IPC channel for lifecycle events
 * Called at agent startup if IPC is enabled
 *
 * @param {string} socketPath - Path to IPC socket
 * @param {string} agentId - Agent identifier
 */
export async function initIPCChannel(socketPath, agentId) {
  if (!socketPath || !agentId) {
    return false;
  }

  try {
    ipcChannel = new AgentChannel(agentId, { socketPath });
    await ipcChannel.connect();

    if (ipcChannel.connected) {
      // Announce presence
      setTimeout(() => {
        if (ipcChannel?.connected) {
          ipcChannel.publish("agent.lifecycle", {
            agent_id: agentId,
            status: "online",
            timestamp: Date.now(),
          }).catch(() => {});
      ...
      // Register IPC disconnect on cleanup
      registerCleanupHandler(async () => {
        if (ipcChannel?.connected) {
          try {
            await ipcChannel.publish("agent.lifecycle", {
              agent_id: agentId,
              status: "offline",
              timestamp: Date.now(),
            });
          ...
          await ipcChannel.close();
        }
      });

      console.error(`[lifecycle] IPC channel initialized for ${agentId}`);
      return true;
    }
  } catch (err) {
    console.error(`[lifecycle] IPC init failed: ${err.message}`);
  }
...
/**
 * Get the current IPC channel (if initialized)
 */
export function getIPCChannel() {
  return ipcChannel;
}
...
function ensureCleanupHooks() {
  ...
  process.on("SIGINT", async () => {
    ...
    // Safety timeout - force exit after 10s
    const forceExitTimer = setTimeout(() => {
      process.stderr.write("Cleanup timeout - forcing exit\n");
      process.exit(130);
    }, 10000);
...
export function cleanupTeamDir(teamName) {
  if (!teamName) {
    process.stderr.write(
      `${colors.yellow}Warning: Cannot cleanup team directory - no team name provided${colors.reset}\n`
    );
    return;
  }
