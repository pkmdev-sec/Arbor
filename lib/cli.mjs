...
function validateAgentArgs(args) {
  ...
  if (args.fallbackModel && !VALID_MODELS.includes(args.fallbackModel))
    errors.push('--fallback-model: must be one of ' + VALID_MODELS.join(', ') + ' — got ' + args.fallbackModel);
  if (errors.length > 0) {
...
export function parseSwarmArgs(argv) {
  const args = {
    ...
    // Fork-merge mode
    forks: 2,  // Number of competing approaches (default: 2)
  ...
  while (i < raw.length) {
    switch (raw[i]) {
      ...
      case "--forks":                  args.forks = Math.min(Math.max(Number(raw[++i]), 2), 5); i++; break;
      case "--debug":                  args.debug = true; i++; break;
      case "-q": case "--quiet":      args.quiet = true; i++; break;
      default:
        if (raw[i].startsWith("-")) {
...
MODES:
  ...
  fork-merge    Generate N approaches → execute in parallel → compare → apply winner
